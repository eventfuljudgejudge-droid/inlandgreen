...
import { blockTransfer } from "@/lib/ledger/transfer.service";
import { errorResponse, serializeTransfer } from "@/lib/api";
import { blockTransferSchema } from "@/lib/ledger/ledger.validation";
...
export async function POST(req: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const parsed = blockTransferSchema.safeParse(body);
    if (!parsed.success) {
    ...
    const transfer = await blockTransfer(id, user.id, parsed.data.reason);
